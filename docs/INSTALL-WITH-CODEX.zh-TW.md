# 用 Codex 一次安裝全部 53 個 Skills

## 推薦方式：把這段完整貼給 Codex

```text
請在這台 Windows 電腦安裝 johnson68878-ops 公開 GitHub 倉庫中的全部 53 個 Codex Skills：
...
3. 執行 scripts/install.ps1，將 manifests/personal-skills.json 中的 53 個 Skill 安裝到目前使用者的 ~/.codex/skills。
...
6. 安裝完成後執行 scripts/verify.ps1，確認 53/53 個 Skill 的 SHA-256 均吻合。
...
```powershell
& "$skillSetup\scripts\Test-SkillsHealth.ps1" -SkillRoots "$env:USERPROFILE\.codex\skills" -ExpectedCount 53 -UseKnownRuntimeRequirements
```
