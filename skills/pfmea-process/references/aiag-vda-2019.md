...
**Failure Cause validation:** Failure Causes must be validated using structured root cause analysis (e.g., [5-Why](../../5why-root-cause/)). Unverified assumptions — "probably," "may be," "could be" — are not acceptable as final Failure Causes. For post-escape PFMEA updates, the FC must match the validated root cause from the 8D or CAPA closure, not a revised team opinion.
